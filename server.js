...
app.post('/api/ai/summarize', async (req, res) => {
    try {
        ...
        try {
            ...
            const prompt = `أنت بروفيسور وأستاذ أطروحات جزائري متميز وخبير حصري في المناهج الرسمية الجزائرية.
الطالب يدرس في المستوى والتخصص: ${levelText}، في مادة: ${subject}.
المطلوب منك كتابة **درس تعليمي تفصيلي وشامل من الصفر** حول عنوان الدرس: "${cleanLesson}".

تعليمات صارمة جداً:
1. **لا تقتصر أبداً على النصائح أو توجيهات الحفظ**؛ بل قم بشرح الدرس كاملاً كأنك تحاضر في قاعة دراسية أو تكتب مرجعاً تعليمياً مفصلاً.
2. ابدأ بمقدمة علمية وتاريخية أو مفاهيمية عميقة لدرس "${cleanLesson}".
3. اشرح بأسلوب مفصل وأكاديمي دقيق جميع المصطلحات، النظريات، القوانين، المبرهنات، والآليات المرتبطة بالدرس مع توضيحها بجداول ومخططات تنظيمية بـ Markdown.
4. اضرب أمثلة تطبيقية رقمية أو واقعية أو برمجية مشروحة ومفصلة خطوة بخطوة.
5. أضف قسم تمارين وتطبيقات شاملة:
   - ### 4️⃣ تمرين تطبيقي مكثف وشامل
   - #### 📝 نص التمرين: (مسألة أو وضعية إدماجية دقيقة)
   - #### 🔑 الحل النموذجي المفصل خطوة بخطوة: (حل كاملاً ومفصلاً بالكامل مع الخطوات والتعليل العلمي).
6. يجب أن يكون الشرح غنياً وطويلاً ومفصلاً باللغة العربية الفصحى دون أي إيجاز أو اختصار مخل.`;
