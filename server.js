...
app.post('/api/ai/summarize', async (req, res) => {
    try {
        ...
        let promptContent = `أنت أستاذ ومساعد تعليمي ذكي ومحترف في المنهج التعليمي الجزائر والوطن العربي.
اكتب ملخصاً شاملاً، تفصيلياً ومبسطاً لمادة "${subject}"، درس "${lesson}".`;

        if (education_level && education_level !== 'غير محدد') {
            promptContent += `\nيرجى تكييف الشرح، مستوى عمق المفاهيم، المصطلحات، وصعوبة التمارين والأسئلة والنصائح بدقة متناهية لتلائم الطالب في المستوى الدراسي: "${education_level}". تأكد أن يكون الملخص مناسباً تماماً لقدراته الاستيعابية ومتطلبات هذا الطور الدراسي.`;
        }

        promptContent += `\nقم بتقسيم الملخص إلى الأقسام التالية بشكل منسق وجذاب:
1. مقدمة مبسطة وممتعة للدرس تثير فضول الطالب.
2. الشرح الكامل والتفصيلي المفاهيمي للدرس مع أمثلة توضيحية واقعية.
3. أهم النقاط والقوانين والتعاريف الأساسية التي يجب حفظها والتركيز عليها.
4. تمرين تطبيقي نموذجي مباشر مع الحل المفصل خطوة بخطوة لترسيخ الفهم.
