...
app.post('/api/ai/summarize', async (req, res) => {
    try {
        ...
        try {
            ...
            const prompt = `أنت أستاذ خصوصي ومربي جزائري خبير ومتمكن جداً في المناهج الرسمية الجزائرية.
الطالب يدرس في المستوى: ${levelText}، في مادة: ${subject}.
قم بشرح وتلخيص الدرس التالي بدقة وعمق: "${cleanLesson}".

تعليمات هامة جداً:
1. ركز بشكل كامل على عنوان الدرس "${cleanLesson}" وقم بتقديم شرح مفصل وأكاديمي بأسلوب مشوق ومحفز "وكأنك تدرس فيه وجهاً لوجه".
2. اشرح المفاهيم الأساسية، القواعد، القوانين أو المحاور بوضوح تام مع أمثلة وجداول أو نقاط منظمة.
3. في نهاية الشرح، **يجب** أن تقدم:
   - قسم بعنوان: ### 4️⃣ تمرين تطبيقي نموذجي (مستوحى من الامتحانات الرسمية)
   - تحته نص التمرين بـ: #### 📝 نص التمرين:
   - ثم الحل المفصل بـ: #### 🔑 الحل المفصل خطوة بخطوة:
4. يجب أن يكون الناتج باللغة العربية الفصحى وبصيغة Markdown الغنية تماماً.`;
