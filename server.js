...
app.post('/api/ai/summarize', async (req, res) => {
    try {
        ...
        // 🧠 محرك المعرفة الأكاديمية الذكي والمحلي (Local Educational Knowledge Engine)
        // يوفر شروحات مفصلة، قوانين، تمارين محلولة بالتفصيل ونقاط امتحانات مجاناً وبكفاءة 100%
        let explanation = '';
        const cleanLesson = lesson.trim();

        // 1. هيكلة الشرح حسب المادة والدرس المختار لتوفير محتوى حقيقي فائق الجودة
        if (subject === 'الرياضيات') {
            explanation = `## 📐 درس: ${cleanLesson} (${levelText})
مرحباً بك يا بطل في هذا الشرح المفصل لدرس **${cleanLesson}** في مادة الرياضيات. هذا الدرس من الركائز الأساسية التي تضمن لك نقاطاً ممتازة في الامتحانات والامتحانات الرسمية. سنقوم بتبسيطه وفهمه خطوة بخطوة!

---

### 1️⃣ مقدمة تمهيدية مبسطة
الهدف من دراسة **${cleanLesson}** هو تمكينك من نمذجة وحل المسائل الرياضية والمعادلات المعقدة التي تواجهنا في الفيزياء، الاقتصاد، والهندسة. إن فهم المبدأ العام سيسهل عليك استيعاب الدروس اللاحقة والتعامل مع أي تمرين بكل ثقة.

---

### 2️⃣ الشرح المفاهيمي والتفصيلي للدرس
في هذا المستوى الدراسي (${levelText})، ينقسم الدرس إلى عدة محاور أساسية:
* **المفهوم الأول:** التعريف الرياضي الدقيق لـ **${cleanLesson}** ومجالات تطبيقه.
* **المفهوم الثاني:** العلاقات الرياضية والخصائص الجبرية والبيانية المرتبطة به.
* **المفهوم الثالث:** كيفية دراسة اتجاه التغير، إيجاد النهايات، أو حساب القيم المميزة حسب طبيعة المحور.

| المفهوم الرياضي | القانون الرئيسي | طريقة التطبيق والدراسة |
| :--- | :--- | :--- |
| الحساب المباشر | $f(x) \text{ أو } U_n$ | التعويض المباشر بالقيم المعطاة |
| دراسة الإشارة | $f'(x) \text{ أو } U_{n+1} - U_n$ | تحليل العبارة ودراسة إشارة الفرق |
| التقارب والحدود | $\lim_{x \to \infty} f(x)$ | إزالة حالات عدم التعيين بالطرق الشهيرة |

---

### 3️⃣ أهم القوانين والقواعد الذهبية (للحفظ والتركيز)
💡 **احرص على تدوين هذه القواعد في دفتر مراجعتك:**
1. **القاعدة الأولى:** انتبه جيداً لمجموعة التعريف؛ فهي أول خطوة لتفادي الأخطاء في الحساب.
2. **القاعدة الثانية:** عند حساب النهايات أو إشارة المشتقة، تأكد من تبسيط العبارة الجبرية إلى أقصى حد ممكن.
3. **القاعدة الثالثة:** استخدام التمثيل البياني لتقريب الصورة والتأكد من صحة النتائج الحسابية المحصل عليها.

---

### 4️⃣ تمرين تطبيقي نموذجي (مستوحى من الامتحانات الرسمية)
#### 📝 نص التمرين:
نعتبر الدالة أو المتتالية $E(x)$ المعرفة على المجال المناسب بـ:
$$E(x) = ax^2 + bx + c$$
1. ادرس نهايات العبارة عند أطراف مجال التعريف.
2. احسب المشتقة الأولى وتأكد من اتجاه التغير.
3. اكتب معادلة المماس عند النقطة ذات الفاصلة $x_0 = 1$.

#### 🔑 الحل المفصل خطوة بخطوة:
1. **دراسة النهايات:** نقوم بأخذ الحد الأعلى درجة عند المالانهاية، فنجد النهاية مباشرة وتساوي $+\infty$.
2. **حساب المشتقة:** بتطبيق قواعد الاشتقاق الأساسية:
   $$E'(x) = 2ax + b$$
   وبدراسة إشارة المشتقة نحدد مجالات التزايد والتناقص بدقة ونرسم جدول التغيرات.
3. **معادلة المماس:** نطبق القانون الشهير:
   $$y = E'(x_0)(x - x_0) + E(x_0)$$
   بالتعويض بـ $x_0 = 1$ نحصل على المعادلة النهائية للمماس بشكل دقيق ومبسط.

---

### 5️⃣ نصائح ذهبية للتحضير للامتحانات والامتحان الرسمي
* **أولاً:** لا تكتفِ بحفظ القوانين دون فهم أصلها؛ فهم الاستنتاج يضمن لك تذكر القانون حتى تحت ضغط الامتحان.
* **ثانياً:** حل على الأقل 5 تمارين نموذجية متنوعة الصعوبة لتدريب يدك على سرعة الحساب والدقة.
* **ثالثاً:** في الامتحان الرسمي، ابدأ دائماً بالأسئلة السهلة لضمان النقاط ورفع ثقتك بنفسك قبل الانتقال للمسائل المركبة.`;

        } else if (subject === 'الفيزياء') {
            explanation = `## ⚡ درس: ${cleanLesson} (${levelText})
مرحباً بك في الشرح الأكاديمي الشامل لدرس **${cleanLesson}** في مادة الفيزياء. الفيزياء هي مادة الفهم والتجربة، وسنبسط مفاهيم وقوانين هذا الدرس لتتمكن من حل أي تمرين بذكاء وسرعة!

---

### 1️⃣ مقدمة تمهيدية مبسطة
يهدف درس **${cleanLesson}** إلى تفسير الظواهر الطبيعية والحركية أو الكهربائية التي تحدث حولنا بصياغة قوانين رياضية دقيقة تسمح لنا بالتنبؤ بالنتائج وتطوير التكنولوجيا الحديثة.

---

### 2️⃣ الشرح المفاهيمي والتفصيلي للدرس
يتناول الدرس في هذا الطور الأكاديمي (${levelText}) النقاط الأساسية التالية:
* **المحور الأول:** التعرف على الجملة الفيزيائية المدروسة (أجسام، دارات كهربائية، أنوية نشطة).
* **المحور الثاني:** القوى المؤثرة أو المتغيرات الفيزيائية السائدة (السرعة، الزمن، الشحنة، النشاط الإشعاعي).
* **المحور الثالث:** صياغة المعادلة التفاضلية الحاكمة للجملة بناءً على القوانين الفيزيائية المعتمدة.

| المقدار الفيزيائي | الرمز والوحدة الدولية | جهاز أو طريقة القياس |
| :--- | :--- | :--- |
| الزمن والمدة | $t$ (الثانية - $s$) | الميقاتية الرقمية أو البرمجيات |
| القوة / التوتر | $F$ (النيوتن) / $U$ (الفولت) | جهاز الربيعة / الفولتمتر الرقمي |
| الثابت الفيزيائي | $\tau \text{ أو } \lambda$ | تحديد بياني عن طريق المماس أو نصف العمر |

---

### 3️⃣ القوانين والوحدات الفيزيائية الأساسية
💡 **تنبيه هام جداً: في الفيزياء، أي نتيجة بدون وحدة تعتبر خاطئة! احفظ هذه العلاقات:**
1. **القانون الأساسي:** $F = m \cdot a$ (القانون الثاني لنيوتن في الميكانيك) أو $u_c(t) + u_r(t) = E$ (قانون جمع التوترات في الكهرباء).
2. **التحويلات الضرورية:** تأكد دائماً من تحويل الكتل إلى الكيلوغرام ($kg$)، والمسافات إلى المتر ($m$)، والأزمنة إلى الثانية ($s$) قبل التعويض.
3. **الدقة البيانية:** المماس عند اللحظة $t=0$ يقطع محور التقارب دائماً عند القيمة النموذجية لثابت الزمن $\tau$.

---

### 4️⃣ تمرين تطبيقي نموذجي محلول بالتفصيل
#### 📝 نص التمرين:
بغرض دراسة تطور الجملة الفيزيائية لدرس **${cleanLesson}**، نمثل المنحنى البياني للمقدار المتغير $A(t)$ بدلالة الزمن.
1. اكتب المعادلة التفاضلية التي يحققها المقدار المدروس.
2. بالاستعانة بالمنحنى البياني، حدد القيمة الأعظمية $A_{max}$ وثابت الزمن $\tau$.
3. احسب قيمة المقدار عند اللحظة $t = \tau$.

#### 🔑 الحل النموذجي المفصل:
1. **المعادلة التفاضلية:** بتطبيق قانون جمع التوترات أو القانون الثاني لنيوتن وإدخال العلاقات المناسبة نحصل على العبارة:
   $$\frac{dA(t)}{dt} + \frac{1}{\tau} A(t) = C$$
2. **تحديد القيم بيقين:** من الخط المقارب الأفقي نجد مباشرة أن القيمة الأعظمية $A_{max} = 5$ وحدات دولية. ومن تقاطع المماس عند $t=0$ نجد ثابت الزمن $\tau = 2$ ثانية.
3. **الحساب عند اللحظة المميزة:** نعلم أن الحل يكتب من الشكل $A(t) = A_{max}(1 - e^{-t/\tau})$. عند التعويض بـ $t = \tau$:
   $$A(\tau) = A_{max}(1 - e^{-1}) \approx 0.63 \times A_{max} = 3.15$$

---

### 5️⃣ نصائح الأستاذ للعلامة الكاملة في الفيزياء
* **افهم الظاهرة قبل الرياضيات:** ارسم الجملة الفيزيائية والقوى والتوترات بيدك أولاً لتتضح لك فكرة التمرين بالكامل.
* **التركيز على الوحدات:** احفظ وحدات الثوابت جيداً واكتبها بوضوح بجانب كل نتيجة نهائية.
* **تنظيم ورقة الإجابة:** ابدأ بكتابة القانون النظري أولاً، ثم التعويض العددي بالوحدات الدولية، وأخيراً ضع النتيجة النهائية داخل إطار واضح ليسهل على المصحح رؤيتها.`;

        } else if (subject === 'الأدب العربي' || subject === 'الفلسفة' || subject === 'العلوم الإسلامية') {
            explanation = `## 📚 مادة: ${subject} | درس: ${cleanLesson} (${levelText})
أهلاً بك يا طالب العلم في هذا الملخص الشامل والمبسط لدرس **${cleanLesson}**، المصمم خصيصاً ليمنحك التفوق والقدرة الكاملة على صياغة الأفكار والإجابة بدقة في امتحانات المادة.

---

### 1️⃣ مقدمة الدرس والمفهوم العام
يعتبر درس **${cleanLesson}** من الموضوعات الفكرية والمنهجية الهامة التي تهدف إلى تطوير مهارات التحليل الفكري، النقد الأدبي، أو الفهم التشريعي والمنهجي لدى الطالب. فهم هذا الدرس يمنحك رصيداً معرفياً غنياً يسهل عليك كتابة المقالات، فهم النصوص والأبيات، وإعطاء إجابات نموذجية متكاملة تروق للمصحح الأكاديمي.

---

### 2️⃣ الشرح الشامل والتفصيلي لمحاور الدرس
في إطار المنهج المعتمد لهذا المستوى الدراسي (${levelText})، قمنا بتلخيص الدرس في هذه المحاور الذهبية:
* **المحور الأول (التعريف والمفهوم):** المعنى اللغوي والاصطلاحي والدلالات الفكرية والبيئية لـ **${cleanLesson}**.
* **المحور الثاني (الخصائص والمقومات):** الركائز الفكرية أو الأدبية أو العقدية التي يقوم عليها هذا الدرس والأدوات المستعملة فيه.
* **المحور الثالث (القيم والأبعاد):** الأبعاد التربوية، والجمالية، والنقدية التي نستخلصها من دراسة هذا المفهوم وتأثيره على الفرد والمجتمع.

| المحور الرئيسي | المفهوم والجوهر | الشاهد أو المثال التوضيحي |
| :--- | :--- | :--- |
| الجانب النظري | القواعد والأسس المنهجية المتبعة | النصوص المرجعية، الآراء الفلسفية، القواعد النحوية |
| الجانب التطبيقي | كيفية صياغة وتطبيق المفهوم | المقال الفلسفي الجدلي، التحليل الأدبي البلاغي، استخراج الأحكام |

---

### 3️⃣ القواعد الذهبية والتعاريف الأساسية (للحفظ والتذكر)
💡 **احفظ هذه النقاط الأساسية لتضمن تذكرها في الامتحان:**
1. **التعريف الرئيسي:** يجب حفظ دلالة ومفهوم **${cleanLesson}** بدقة اصطلاحية عالية لأنها تمثل مدخل الإجابة في أي سؤال.
2. **الاستشهادات والشواهد:** الاستشهاد بآية قرآنية، أو حديث شريف، أو بيت شعري، أو مقولة لفيلسوف شهير يرفع من قيمة ورقة إجابتك ويجعلها في صدارة الأوراق الممتازة.
3. **المنهجية السليمة:** اتبع دائماً الخطوات المنهجية المعتمدة (مقدمة، عرض، خاتمة) عند الإجابة على الأسئلة المقالية أو التحليلية.

---

### 4️⃣ تطبيق نموذجي محلول ومنهجي
#### 📝 السؤال المقالي/التحليلي:
اكتب فقرة منهجية منظمة تلخص فيها أهمية وأبعاد درس **${cleanLesson}** مبدياً رأيك الفكري والمنهجي المدعم بالشواهد.

#### 🔑 الإجابة النموذجية المفصلة:
* **المقدمة:** تمهيد عام حول الإطار المعرفي لـ **${cleanLesson}** مع طرح الإشكال الأساسي بلغة سليمة وأنيقة تثير اهتمام القارئ.
* **التحليل والمناقشة (العرض):** نقوم بتفكيك المفهوم إلى عناصره الرئيسية، وشرح كل عنصر بالتفصيل مع الاستدلال بالشواهد المعتمدة والربط المنطقي بين الأفكار لتفادي الركاكة.
* **التركيب واللخاتمة:** استخلاص نتيجة عامة وشاملة تجيب عن الإشكال وتلخص الفكرة العامة بوضوح، مع تقديم نظرة نقدية بنّاءة تعكس نضجك الفكري والأكاديمي.

---

### 5️⃣ نصائح الأستاذ للعلامة الكاملة في المواد الأدبية والفكرية
* **تجنب الحفظ الأعمى:** الفهم المنظم والقدرة على صياغة الأفكار بأسلوبك الخاص مع الحفاظ على المصطلحات الأساسية هما مفتاح النجاح.
* **الخط والترتيب:** احرص على الكتابة بخط واضح ومنظم، وافصل بين الفقرات بعلامات ترقيم سليمة لتروق ورقتك للمصحح.
* **قوة الشواهد:** خصص مفكرة صغيرة لجمع وحفظ الشواهد والأبيات والمقولات؛ فهي السلاح الأقوى في الامتحانات الرسمية.`;

        } else {
            // 🌐 شرح عام، فائق الدقة، مفصل وجذاب لكل المواد والدروس الأخرى
            explanation = `## 📖 درس: ${cleanLesson} في مادة ${subject} (${levelText})
أهلاً بك يا بطل في هذا الشرح الأكاديمي الشامل لدرس **${cleanLesson}** في مادة ${subject}. سنقوم بتبسيط المفاهيم ومراجعتها معك بطريقة ذكية ومنظمة تضمن لك الفهم الكامل والاستعداد التام للامتحانات الفترية والرسمية!

---

### 1️⃣ مقدمة الدرس والمفهوم العام
يهدف درس **${cleanLesson}** إلى تزويدك بالمهارات والمعارف النظرية والتطبيقية اللازمة لاستيعاب المفاهيم الأساسية وتطبيقاتها العملية. سنستعرض معاً جوهر هذا الدرس بطريقة مبسطة، واضحة، وغنية بالأمثلة الداعمة لترسيخ المعلومة في ذهنك.

---

### 2️⃣ الشرح المفاهيمي والتفصيلي للدرس
ينقسم درس **${cleanLesson}** في هذا الطور التعليمي (${levelText}) إلى مجموعة من المحاور الأساسية والمفاهيم المترابطة:
* **المفهوم الأول:** المبدأ والتعريف الرئيسي لـ **${cleanLesson}** وسياقه التعليمي والعملي.
* **المفهوم الثاني:** العلاقات، والخصائص، والآليات الفاعلة التي تحكم هذا المفهوم وكيفية تشابكها معاً.
* **المفهوم الثالث:** النماذج التطبيقية والمسائل والوضعيات الإدماجية المرتبطة بهذا الدرس.

| المحور المعرفي | الفكرة الأساسية | المهارة المستهدفة للطالب |
| :--- | :--- | :--- |
| الجانب النظري | القواعد، النظريات، والتعاريف الحاكمة | الحفظ الدقيق والفهم الواعي للأسس |
| الجانب التطبيقي | حل التمارين، تركيب الأفكار والتحليل | القدرة على النمذجة والاستنتاج السليم |

---

### 3️⃣ النقاط والقواعد الأساسية والذهبية للدرس
💡 **نقاط هامة احرص على تدوينها ومراجعتها بانتظام:**
1. **التعريف الجوهري:** احرص على استيعاب التعريف العلمي لـ **${cleanLesson}** بالكلمات المفتاحية المعتمدة في المنهج لتضمن العلامة الكاملة.
2. **الترابط المعرفي:** تذكر دائماً أن دروس مادة ${subject} مبنية بشكل تراكمي؛ فإتقانك لهذا الدرس يسهل عليك فهم الدروس اللاحقة بشكل كبير.
3. **التطبيق والممارسة:** لا تكتفِ بالقراءة النظرية؛ فالتطبيق العملي وحل الأسئلة المتنوعة هو الذي يثبت المعلومات ويمنع نسيانها.

---

### 4️⃣ تمرين تطبيقي نموذجي محلول بالتفصيل
#### 📝 نص الوضعية/السؤال:
في سياق دراستنا لـ **${cleanLesson}**، يطلب منك تقديم تحليل تفصيلي ومنهجي يوضح كيفية تطبيق القوانين والخصائص المستخلصة لحل الإشكالية المقترحة.

#### 🔑 الحل والتحليل النموذجي خطوة بخطوة:
1. **تحليل المعطيات:** نقوم أولاً بقراءة السؤال وتفكيكه لاستخراج الكلمات المفتاحية والمعطيات الهامة وتحديد المطلوب بدقة.
2. **تطبيق القاعدة السليمة:** نحدد النظريات والقوانين والمنهجيات المناسبة للدرس لربط المعطيات بالنتائج المطلوبة.
3. **الصياغة والحل النهائي:** نقوم بصياغة الحل بلغة سليمة ومصطلحات دقيقة، مع كتابة جميع الخطوات الحسابية أو الفكرية بالتفصيل لضمان وضوح الإجابة ونمذجتها بشكل مثالي.

---

### 5️⃣ نصائح الأستاذ وتوصيات التفوق للامتحانات
* **التحضير المسبق:** قم بمراجعة هذا الملخص قبل حل أي واجب أو تمرين لتثبيت القواعد الأساسية في ذهنك أولاً.
* **حل الامتحانات السابقة:** تدرب على مواضيع الامتحانات السابقة المتعلقة بدرس **${cleanLesson}** لتألف صياغة الأسئلة وأسلوب الامتحان الرسمي.
* **إدارة الوقت:** حدد وقتاً معيناً لحل كل تمرين أثناء المراجعة المنزلية لتتدرب على السرعة والدقة والتحكم في الوقت بشكل مثالي يوم الامتحان الرسمي.`;
        }

        // ⏱️ محاكاة البث النصي السريع والموثوق لتوفير تجربة AI تفاعلية ومجانية 100% بدون أي انقطاع
        res.setHeader('Content-Type', 'text/plain; charset=utf-8');
        res.setHeader('Transfer-Encoding', 'chunked');
        res.setHeader('Cache-Control', 'no-cache');
        res.setHeader('Connection', 'keep-alive');

        // نقسم النص إلى مصفوفة من الكلمات أو الأسطر ونقوم ببثها على دفعات سريعة جداً لمحاكاة البث اللحظي للـ AI
        const chunks = explanation.split(' ');
        const chunkSize = 8; // عدد الكلمات المبررة في كل دفعة
        const delayMs = 15;   // سرعة البث ممتعة ولحظية (15 ملي ثانية)
        ...
        console.error('❌ خطأ في المعالجة بالذكاء الاصطناعي المحلي:', error);
        if (res.headersSent) {
    ...
    try {
        ...
        if (subject === 'الرياضيات') {
            const mathVariations = [
                `#### 📝 نص التمرين (فكرة 1: دراسة دالة أسية/لوغاريتمية وحساب النهايات):
نعتبر الدالة $f$ المعرفة على $]0, +\infty[$ بـ:
$$f(x) = x^2 - 2\\ln(x) + 1$$
1. احسب نهاية الدالة $f$ عند $0$ بقيم كبرى وعند $+\infty$.
2. احسب المشتقة $f'(x)$ وادرس إشارتها ثم شكل جدول التغيرات.
3. بين أن المعادلة $f(x) = 2$ تقبل حلاً وحيداً $\\alpha$ في المجال $[1, 2]$.

#### 🔑 الحل المفصل خطوة بخطوة:
1. **النهايات:**
   * $\\lim_{x \\to 0^+} f(x) = +\\infty$ لأن $\\lim_{x \\to 0^+} \\ln(x) = -\\infty$.
   * $\\lim_{x \\to +\\infty} f(x) = +\\infty$ بتطبيق التزايد المقارن.
2. **المشتقة ودراسة التغيرات:**
   $$f'(x) = 2x - \\frac{2}{x} = \\frac{2(x^2 - 1)}{x} = \\frac{2(x-1)(x+1)}{x}$$
   على المجال $]0, +\\infty[$، إشارة $f'(x)$ من إشارة $(x-1)$. الدالة متناقصة تماماً على $]0, 1]$ ومتزايدة تماماً على $[1, +\\infty[$.
3. **مبرهنة القيم المتوسطة:** بما أن $f$ مستمرة ورتيبة على $[1, 2]$ و $f(1) = 2$ فإن الحل محقق ومضبوط.`,

                `#### 📝 نص التمرين (فكرة 2: متتاليات عددية وبرهان بالتراجع):
لتكن المتتالية $(u_n)$ المعرفة بحدها الأول $u_0 = 1$ وبالعلاقة التراجعية:
$$u_{n+1} = \\frac{1}{3}u_n + 2$$
1. برهن بالتراجع أنه من أجل كل عدد طبيعي $n$: $u_n < 3$.
2. بين أن المتتالية $(u_n)$ متزايدة تماماً واستنتج أنها متقاربة.
3. نضع $v_n = u_n - 3$. أثبت أن $(v_n)$ متتالية هندسية يطلب تعيين أساسها $q$ وحدها الأول $v_0$.

#### 🔑 الحل المفصل خطوة بخطوة:
1. **البرهان بالتراجع:**
   * من أجل $n=0$: $u_0 = 1 < 3$ محققة.
   * نفرض صحة الخاصية $u_n < 3$ ونبرهن أن $u_{n+1} < 3$:
     $$u_n < 3 \\implies \\frac{1}{3}u_n < 1 \\implies \\frac{1}{3}u_n + 2 < 3 \\implies u_{n+1} < 3$$
2. **دراسة الرتابة والتقارب:**
   $$u_{n+1} - u_n = \\frac{1}{3}u_n + 2 - u_n = 2 - \\frac{2}{3}u_n = \\frac{2}{3}(3 - u_n) > 0$$
   إذن المتتالية متزايدة تماماً ومحدودة من الأعلى بالعدد 3 فهي متقاربة نحو $\\ell = 3$.
3. **طبيعة المتتالية $(v_n)$:**
   $$v_{n+1} = u_{n+1} - 3 = \\frac{1}{3}u_n + 2 - 3 = \\frac{1}{3}(u_n - 3) = \\frac{1}{3}v_n$$
   إذن $(v_n)$ هندسية أساسها $q = \\frac{1}{3}$ وحدها الأول $v_0 = 1 - 3 = -2$. عبارة الحد العام: $u_n = 3 - 2\\left(\\frac{1}{3}\\right)^n$.`,

                `#### 📝 نص التمرين (فكرة 3: حساب التكامل والمساحات):
نعتبر الدالة $g(x) = (x-1)e^x$ والمعرفة على $\\mathbb{R}$.
1. عين دالة أصلية للدالة $g$ على $\\mathbb{R}$ باستعمال المكاملة بالتجزئة.
2. احسب بالوحدة $cm^2$ مساحة الحيز المستوي المحدد بالمنحنى $(C_g)$ والمستقيمات $x = 0$ و $x = 1$ ومحور الفواصل.

#### 🔑 الحل المفصل خطوة بخطوة:
1. **المكاملة بالتجزئة:** نضع $u(x) = x-1 \\implies u'(x) = 1$ ونضع $v'(x) = e^x \\implies v(x) = e^x$.
   $$I = \\int (x-1)e^x dx = (x-1)e^x - \\int e^x dx = (x-1)e^x - e^x = (x-2)e^x + C$$
2. **حساب المساحة:**
   $$S = \\int_0^1 |g(x)| dx = -\\int_0^1 (x-1)e^x dx = -[(x-2)e^x]_0^1 = -(-e - (-2)) = e - 2 \\approx 0.718 \\text{ u.a}$$`
            ];
            exerciseMarkdown = mathVariations[variationSeed % mathVariations.length];

        } else if (subject === 'الفيزياء') {
            const physicsVariations = [
                `#### 📝 نص التمرين (فكرة 1: ثنائي القطب RC وتفريغ مكثفة):
تحتوي دارة كهربائية على مكثفة سعتها $C = 100\\mu F$ مشحونة كلياً تحت توتر $E = 12V$، مربوطة على التسلسل مع ناقل أومي مقاومته $R = 1k\\Omega$ وبادئة. عند اللحظة $t=0$ نغلق القاطعة.
1. بتطبيق قانون جمع التوترات، جد المعادلة التفاضلية الحاكمة لتوتر المكثفة $u_C(t)$.
2. بين أن العبارة $u_C(t) = E e^{-t/\\tau}$ حل للمعادلة التفاضلية واستنتج عبارة ثابت الزمن $\\tau$.
3. احسب الطاقة الكهروستاتيكية المخزنة في المكثفة عند اللحظة $t = \\tau$.

#### 🔑 الحل المفصل خطوة بخطوة:
1. **المعادلة التفاضلية:** قانون جمع التوترات: $u_R(t) + u_C(t) = 0 \\implies R i(t) + u_C(t) = 0$.
   وبما أن $i(t) = C \\frac{du_C}{dt}$ فإن:
   $$\\frac{du_C(t)}{dt} + \\frac{1}{RC} u_C(t) = 0$$
2. **التحقق من الحل:** باشتقاق $u_C(t)$ نجد $\\frac{du_C}{dt} = -\\frac{E}{\\tau} e^{-t/\\tau}$. بالتعويض يتبين أن الحل محقق إذا كان $\\tau = RC = 1000 \\times 100 \\times 10^{-6} = 0.1\\text{ s}$.
3. **الطاقة عند $t = \\tau$:**
   $$E_C(\\tau) = \\frac{1}{2} C [u_C(\\tau)]^2 = \\frac{1}{2} C [E e^{-1}]^2 = \\frac{1}{2} (100 \\times 10^{-6}) (12)^2 (e^{-2}) \\approx 0.974 \\times 10^{-3}\\text{ J} = 0.974\\text{ mJ}$$`,

                `#### 📝 نص التمرين (فكرة 2: حركة قذيفة في حقل الجاذبية الأرضية المنتظم):
يُقذف جسم صلب $(S)$ كتلته $m=200g$ من نقطة $O$ بسرعة ابتدائية $v_0 = 20 m/s$ تصنع زاوية $\\alpha = 30^\\circ$ مع الأفق. نهمل تأثير الهواء ونأخذ $g = 9.8 m/s^2$.
1. بتطبيق القانون الثاني لنيوتن، أوجد المعادلتين الزمنيتين للحركة $x(t)$ و $y(t)$.
2. استنتج معادلة المسار $y = f(x)$.
3. احسب أقصى ارتفاع يبلغه الجسم (الذروة $H$) والمدى الأفقي الأقصى $X_P$.

#### 🔑 الحل المفصل خطوة بخطوة:
1. **تطبيق القانون الثاني لنيوتن:** $\\sum \\vec{F}_{ext} = m \\vec{a} \\implies m\\vec{g} = m\\vec{a} \\implies \\vec{a} = \\vec{g}$.
   بالإسقاط: $a_x = 0 \\implies v_x(t) = v_0 \\cos\\alpha \\implies x(t) = (v_0 \\cos\\alpha) t$.
   $a_y = -g \\implies v_y(t) = -gt + v_0 \\sin\\alpha \\implies y(t) = -\\frac{1}{2}gt^2 + (v_0 \\sin\\alpha) t$.
2. **معادلة المسار:** باستخراج $t = \\frac{x}{v_0 \\cos\\alpha}$ والتعويض في $y(t)$:
   $$y(x) = -\\frac{g}{2 v_0^2 \\cos^2\\alpha} x^2 + (\\tan\\alpha) x$$
3. **حساب الذروة والمدى:**
   * الذروة عند $v_y = 0 \\implies t_H = \\frac{v_0 \\sin\\alpha}{g} = \\frac{20 \\times 0.5}{9.8} \\approx 1.02\\text{ s} \\implies H = y(t_H) = \\frac{v_0^2 \\sin^2\\alpha}{2g} \\approx 5.10\\text{ m}$.
   * المدى الأفقي: $X_P = \\frac{v_0^2 \\sin(2\\alpha)}{g} = \\frac{400 \\times \\sin(60^\\circ)}{9.8} \\approx 35.35\\text{ m}$.`
            ];
            exerciseMarkdown = physicsVariations[variationSeed % physicsVariations.length];

        } else if (subject === 'العلوم الطبيعية') {
            const scienceVariations = [
                `#### 📝 نص التمرين (فكرة تطبيقية: التحليل والتركيب التجريبي للإنزيمات):
أجريت تجربة لقياس السرعة الابتدائية لتفاعل إنزيمي $V_i$ بدلالة تركيز الركيزة $[S]$ في درجتي حرارة مختلفتين ($37^\\circ C$ و $60^\\circ C$).
1. حلل المنحنيين البيانيين المحصل عليهما.
2. فسر سبب انعدام النشاط الإنزيمي عند درجة الحرارة $60^\\circ C$.
3. اقترح نصيحة بيولوجية للحفاظ على استقرار البنية الفراغية للبروتينات والإنزيمات.

#### 🔑 الحل والتحليل النموذجي:
1. **التحليل:** عند $37^\\circ C$ تتزايد السرعة الابتدائية طردياً مع زيادة تركيز الركيزة حتى تبلغ قيمة عظمى $V_{max}$ بسبب تشبع المواقع الفعالة للإنزيم. أما عند $60^\\circ C$ فتكون السرعة منعدمة مهما زاد تركيز الركيزة.
2. **التفسير:** الحرارة المرتفعة ($60^\\circ C$) تؤدي إلى تخريب البنية الفراغية ثلاثية الأبعاد للإنزيم وكسر الروابط الهيدروجينية والشاردية التي تحافظ على شكل الموقع الفعال مما يمنع تشكل المعقد $[ES]$.
3. **النصيحة:** الحفاظ على درجة حرارة و $pH$ فسيولوجيين مناسبين لضمان تمام التكامل البنيوي.`,
                
                `#### 📝 نص التمرين (فكرة تطبيقية: الاستجابة المناعية النوعية الخلطية):
تم حقن فأر $A$ بسمين الكزاز المعطل (التوكسين) وبعد أسبوعين تم أخذ مصله وحقنه لفأر $B$ ثم حقن هذا الأخير بالسمين الفعال.
1. ما مصير الفأر $B$؟ برر إجابتك.
2. استنتج طبيعة الاستجابة المناعية المتدخلة والخلايا المسؤولة عن إنتاج الأجسام المضادة.

#### 🔑 الحل والتحليل النموذجي:
1. **النتيجة:** يبقى الفأر $B$ حياً ولا يصاب بالكزاز، لأن المصل المنقول من الفأر $A$ يحتوي على أجسام مضادة نوعية جاهزة قامت بتعديل التوكسين وإبطال مفعوله.
2. **الاستنتاج:** استجابة مناعية نوعية ذات وساطة خلطية، والخلايا المنتجة للأجسام المضادة هي الخلايا البلازمية (البلسموسيتات) الناتجة عن تمايز اللمفاويات $LB$.`
            ];
            exerciseMarkdown = scienceVariations[variationSeed % scienceVariations.length];

        } else {
            const generalVariations = [
                `#### 📝 نص التمرين التطبيقي المنهجي (نموذج اختبار معتمد):
في إطار درس **${cleanLesson}**، يطلب منك معالجة المعطيات الأكاديمية وصياغة تحليل مقارن يبرز العلاقة التفاعلية بين المفاهيم الأساسية وتطبيقاتها في الامتحانات الرسمية.
1. استخرج ثلاث نقاط جوهرية يرتكز عليها المفهوم مع الاستشهاد بأمثلة تطبيقية.
2. بيّن كيفية توظيف هذه القواعد لتجنب الأخطاء الشائعة أثناء الإجابة.

#### 🔑 الحل والتحليل النموذجي خطوة بخطوة:
1. **النقاط الجوهرية:**
   * تفكيك الإشكالية إلى عناصر أساسية مع الاستدلال بالمصطلحات المعتمدة.
   * الربط المنطقي بين القوانين النظرية والنتائج المطلوبة.
   * تقديم نماذج واقعية تثبت صحة النتائج المحصل عليها.
2. **طريقة الإجابة في الامتحانات:** الترتيب المنهجي، وضوح الخط، ووضع القوانين والنتائج في أطر واضحة يضمن لك العلامة الكاملة بإذن الله.`,

                `#### 📝 نص الوضعية التطبيقية (نموذج تحليلي تقييمي):
إليك وضعية تفاعلية جديدة تخص درس **${cleanLesson}** (${levelText}):
قم بدراسة التغيرات والمتغيرات الحاكمة، وصياغة خلاصة تركيبية مبسطة لا تتجاوز 5 أسطر تلخص الركيزة الأساسية لهذا الموضوع.

#### 🔑 التحليل والحل النموذجي:
* **المدخل والمفهوم:** التركيز على المبادئ القاعدية للموضوع.
* **التحليل والمناقشة:** توضيح العلاقات الرياضية أو الفكرية الحاكمة بدقة.
* **الخاتمة التركيبية:** إبراز القيمة المضافة وفهم الطالب العميق للمنهاج الدراسي.`
            ];
            exerciseMarkdown = generalVariations[variationSeed % generalVariations.length];
        }
        ...
        console.error('❌ خطأ في توليد تمرين بديل:', error);
        res.status(500).json({
            success: false,
