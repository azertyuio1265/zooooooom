...
app.post('/api/ai/summarize', async (req, res) => {
    try {
        ...
        try {
            ...
            const prompt = `أنت بروفيسور وأستاذ خصوصي جزائري خبير ومتمكن جداً في المناهج الرسمية الجزائرية (مدرس في الثانويات النموذجية، الجامعات، ومعاهد التكوين المهني INSFP).
الطالب يدرس في المستوى والتخصص: ${levelText}، في مادة: ${subject}.
المطلوب منك شرح الدرس التالي بدقة تامة وعمق أكاديمي بالغ ومفصل جداً: "${cleanLesson}".

تعليمات صارمة للشرح:
1. قدم شرحاً مطولاً، عميقاً، وشاملاً (بمثابة محاضرة أو درس كامل في المرجع المعتمد) بأسلوب مشوق ومحفز وكأنك تدرس الطالب وجهاً لوجه.
2. اشرح المفاهيم الأساسية، المصطلحات العلمية، النظريات، والقوانين مع تفصيل كل عنصر بوضوح تام مستعيناً بجداول، نقاط منظمة، ومخططات توضيحية بـ Markdown.
3. قدم أمثلة تطبيقية واقعية أو رياضية مشروحة خطوة بخطوة.
4. في نهاية الدرس، **يجب** أن تقدم:
   - قسم بعنوان: ### 4️⃣ تمرين تطبيقي نموذجي متقدم (مستوحى من الامتحانات الرسمية)
   - تحته نص التمرين بـ: #### 📝 نص التمرين:
   - ثم الحل المفصل والشافي بخطوات رياضية أو تقنية أو نظرية دقيقة: #### 🔑 الحل المفصل خطوة بخطوة:
5. أضف قسمًا خاصًا: ### 5️⃣ نصائح الأستاذ الخبير للتميز وضمان العلامة الكاملة.
6. يجب أن يكون الشرح باللغة العربية الفصحى وبصيغة Markdown الغنية جداً دون أي اختصار مخل.`;
            ...
            console.warn('⚠️ تفعيل محرك الطوارئ الاحتياطي للشرح (تجاوز الحصة أو خطأ API):', aiErr.message);
            explanation = `## 👨‍🏫 شرح الأستاذ الخبير: ${cleanLesson} (${subject} - ${levelText})

أهلاً بك يا بطل في درس **${cleanLesson}** لمادة **${subject}** (${levelText}). بصفتي أستاذك الخبير في المنهاج الجزائري، سأبسط لك هذا الدرس بطريقة ممتعة وواضحة لتضمن التفوق والامتياز!

---

### 1️⃣ مقدمة تمهيدية مبسطة
يُعتبر درس **${cleanLesson}** من أهم الركائز المعرفية في المنهاج الرسمي الجزائري. الهدف الأساسي من دراسته هو تمكينك من استيعاب المبادئ والنظريات الأساسية وتطبيقها بكل ثقة في الفروض والامتحانات الرسمية.

---

### 2️⃣ الشرح المفاهيمي والتفصيلي للدرس
في هذا المستوى الدراسي، ينقسم الدرس إلى المحاور الأساسية التالية:
* **المفهوم الأول:** التعريف الدقيق والمصطلحات العلمية المعتمدة لـ **${cleanLesson}**.
* **المحور الثاني:** القواعد، العلاقات، والخصائص التنظيمية والرياضية أو العلمية المرتبطة به.
* **المحور الثالث:** التطبيق العملي ونمذجة الحلول لمواجهة الوضعيات الإدماجية المختلفة.

| المحور المعرفي | القاعدة أو القانون الأساسي | طريقة التطبيق النموذجي |
| :--- | :--- | :--- |
| الأساس النظري | التعريف والخصائص الجوهرية | الحفظ الواعي والفهم العميق للمصطلحات |
| التطبيق العملي | النظريات والقوانين المرتبطة | التوظيف المباشر في التمارين |

---

### 3️⃣ القواعد الذهبية والنقاط الأساسية للحفظ
💡 **احفظ هذه التوجيهات لتضمن العلامة الكاملة:**
1. **الضبط الاصطلاحي:** احرص على كتابة التعريف العلمي الدقيق لـ **${cleanLesson}** كما ورد في المقرر الرسمي.
2. **الترابط المنهجي:** اربط بين معطيات السؤال والقوانين المدروسة لتحديد طريقة الحل بدقة.
3. **التنظيم والدقة:** نظم إجابتك في ورقة الامتحان بوضع القوانين والنتائج داخل أطر واضحة.

---

### 4️⃣ تمرين تطبيقي نموذجي (مستوحى من الامتحانات الرسمية)
#### 📝 نص التمرين:
نعتبر الإشكالية أو التطبيق المباشر التالي المتعلق بدرس **${cleanLesson}** (${subject}):
1. قدم تحليلاً مفصلاً لمكونات وعناصر الدرس.
2. أوجد الحل النموذجي بالاستعانة بالقواعد والنظريات المقررة.

#### 🔑 الحل المفصل خطوة بخطوة:
1. **الخطوة الأولى:** تفكيك المعطيات الأساسية واستخراج المطلوب بدقة متناهية.
2. **الخطوة الثانية:** استدعاء القاعدة أو القانون المناسب والتعويض العددي أو الفكري السليم.
3. **الخطوة الثالثة:** صياغة النتيجة النهائية بوضوح مع إبراز الوحدات والمصطلحات العلمية المناسبة.

---

### 5️⃣ نصائح الأستاذ للتميز
* **التدريب المنتظم:** حل أكبر عدد ممكن من التمارين والوضعيات التطبيقية.
* **إدارة الوقت:** تدرب على حل المواضيع في توقيت مماثل لوقت الامتحان الرسمي.`;
        }
  ...
  "subjects": [
    {
      ...
      "units": ["الوحدة 1", "الوحدة 2", "الوحدة 3"]
            ...
            console.warn('⚠️ تفعيل الاستجابة الاحتياطية المخصصة للمنهاج الجزائري:', aiErr.message);
            const title = `${stage || ''} ${grade || ''} ${branch || ''} ${specialty || levelName || ''}`.trim() || 'المنهاج الجزائري الرسمي';
            
            let defaultSubjects = [];
            const st = (stage || '').toLowerCase();
            if (st.includes('primary') || st.includes('ابتدائي')) {
                defaultSubjects = [
                    { name: "اللغة العربية", icon: "📖", units: ["التعبير الشنوي والقراءة", "الصياغة النحوية والتصريف", "التعبير الكتابي والإنشاء"] },
                    { name: "الرياضيات", icon: "🔢", units: ["الأعداد والعمليات الحسابية", "الفضاء والمهندسات", "قياس المقادير وحل المشكلات"] },
                    { name: "التربية الإسلامية", icon: "🕌", units: ["السيرة النبوية العطرة", "القرآن الكريم والأحكام", "القيم والاخلاق"] },
                    { name: "التربية العلمية", icon: "🌱", units: ["الكائنات الحيّة وبيئتها", "صحة الإنسان والغذاء", "المادة والكهرباء المحيطة بنا"] }
                ];
            } else if (st.includes('middle') || st.includes('متوسط')) {
                defaultSubjects = [
                    { name: "الرياضيات", icon: "📐", units: ["الحساب على الجذور", "الحساب الحرفي والمعادلات", "المعلم في المستوي ودائرة الأطوال"] },
                    { name: "الفيزياء وعلوم الطور", icon: "⚡", units: ["الظواهر الكهربائية", "المادة و تحولاتها", "الميكانيك والقوى"] },
                    { name: "اللغة العربية", icon: "📚", units: ["قضايا اجتماعية وإنسانية", "الفن والرياضة", "العلم والتكنولوجيا"] },
                    { name: "العلوم الطبيعية", icon: "🧬", units: ["التغذية عند الإنسان", "تحويل الأغذية والهضم", "التكاثر عند النباتات"] },
                    { name: "التاريخ والجغرافيا", icon: "🌍", units: ["التاريخ المعاصر للجزائر", "الوسط الطبيعي والسكان", "التنمية الاقتصادية"] }
                ];
            } else if (st.includes('vocational') || st.includes('تكوين') || st.includes('insfp')) {
                defaultSubjects = [
                    { name: "تحليل وتصميم نظم المعلومات", icon: "💻", units: ["مخططات حالات الاستخدام UML", "مخططات التسلسل والنشاط", "تصميم واجهات المستخدم الحديثة"] },
                    { name: "قواعد البيانات المتقدمة SQL", icon: "🗄️", units: ["لغة الاستعلام المهيكلة SQL", "العلاقات والقيود والتطبيع", "إدارة وتأمين قواعد البيانات"] },
                    { name: "البرمجة بلغة جافาสكريبت ونود", icon: "⚡", units: ["أساسيات البرمجة غير المتزامنة", "بناء خوادم الويب بـ Express", "ربط الواجهات الخلفية بقواعد البيانات"] },
                    { name: "أمن الشبكات وأنظمة التشغيل", icon: "🔒", units: ["أساسيات أمن الحوسبة", "إدارة خوادم Linux", "النسخ الاحتياطي وحماية البيانات"] }
                ];
            } else if (st.includes('university') || st.includes('جامعي') || st.includes('lmd')) {
                defaultSubjects = [
                    { name: "الخوارزميات وهياكل البيانات المعقدة", icon: "📊", units: ["هياكل البيانات الخطية وغير الخطية", "خوارزميات البحث والترتيب المتقدمة", "تحليل تعقيد الخوارزميات"] },
                    { name: "قواعد البيانات الموزعة", icon: "🗄️", units: ["تصميم الأنظمة الموزعة", "إدارة المعاملات وقفل البيانات", "النسخ المتماثل والتوافر العالي"] },
                    { name: "هندسة البرمجيات المتقدمة", icon: "⚙️", units: ["أنماط التصميم Design Patterns", "اختبار البرمجيات وتأكيد الجودة", "إدارة مشاريع البرمجيات بـ Agile"] }
                ];
            } else {
                // Secondary default
                defaultSubjects = [
                    { name: "الرياضيات", icon: "📐", units: ["الدوال العددية", "المتتاليات العددية", "الإحتمالات", "الأعداد المركبة"] },
                    { name: "الفيزياء", icon: "⚡", units: ["تطور جملة كيميائية", "الميكانيك والظواهر الكهربائية", "التحولات النووية"] },
                    { name: "العلوم الطبيعية", icon: "🧬", units: ["المصادقة على التركيب البروتيني", "التكتونية العامة", "المناعة"] },
                    { name: "الأدب العربي", icon: "📚", units: ["الشعر التعليمي والمديح", "النثر العلمي المتأسك", "المدرسة العاطفية والالتزام"] },
                    { name: "الفلسفة", icon: "💭", units: ["الشعور واللاشعور", "العاطفة والإنفعال", "الذاكرة والتخيل"] },
                    { name: "اللغات الأجنبية", icon: "🌐", units: ["Ethics in Business", "Astrophysics & Science", "Advertising & Media"] }
                ];
            }
