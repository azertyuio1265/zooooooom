...
export declare interface IAgoraRTC extends EventEmitter {
     ...
     * > 注意事项：
     * > - 一个频道内的所有用户必须使用同样类型的 `uid`，即必须都为整数或都为字符串。
     * > - 使用字符串作为用户 ID 支持与 Native SDK 2.8 及以上版本互通，请确保 Native SDK 使用 User Account 加入频道。详见[使用 String 型的用户名](https://doc.shengwang.cn/faq/integration-issues/string-uid)。
     * > - 为保证水晶球数据的准确性，建议自行指定 `uid` 并保证唯一性。
     */
     ...
     * - {@link ScreenAudioTrackInitConfig}: 自定义音频共享的初始化配置，包含 3A 处理参数 (`AEC`、`AGC`、`ANS`）和本地音频播放设置。
     * > 注意事项：
     ...
     * - `disable`: （默认）不分享音频。
     * - `auto`: 根据浏览器是否支持决定是否分享音频。
     * - {@link ScreenAudioTrackInitConfig}: 自定义音频共享的初始化配置，包含 3A 处理参数 (`AEC`、`AGC`、`ANS`) 和本地音频播放及采集设置。
     * > - 该功能仅适用于支持 Web SDK 的浏览器桌面端，不支持移动端。具体支持列表详见[浏览器兼容性和已知问题](https://doc.shengwang.cn/doc/rtc/javascript/overview/browser-compatibility)。
     * > - 关于各操作系统平台的浏览器版本及功能支持的补充说明如下：
     * >   - 在 macOS 平台上，Chrome 74 及以上版本支持音视频共享，仅支持在共享 Chrome 标签页时分享音频。Firefox 和 Safari 14 及以上版本支持窗口和屏幕共享，不支持分享音频。
     * >   - 在 Windows 平台上，Chrome 74 及以上版本和 Edge 支持在共享屏幕和浏览器标签页时分享音频，不支持在共享应用窗口时分享音频。Firefox 支持窗口和屏幕共享，不支持分享音频。
     ...
     *
     * > 开启日志上传后，SDK 会在调用 `join` 方法 5 秒后开始上传日志，无需等待加入频道成功。
     * @category Logger
     */
...
export declare interface IAgoraRTCClient extends EventEmitter {
     ...
     * > - 为保证水晶球数据的准确性，建议自行指定 `uid` 并保证唯一性。
     *
     * @param options 加入频道选项。详见 [IJoinOptions]{@link IJoinOptions}。
     *
     ...
     *
     * 和原来的 {@link enableDualStream}/{@link disableDualStream} 相比：
...
export declare interface IJoinOptions {
    ...
    autoSubscribe?: boolean;
    /**
     * 是否在加入频道时自动接收并尝试播放远端音频。
     * - `true`：是
     * - `false`：（默认）否
     *
     * 设置该参数为 `true` 后，SDK 会在用户加入频道时自动接收并尝试播放远端音频，减少入会后等待声音出现的时间。
     *
     * 该功能适用于语聊房、在线会议等需要入会后尽快听到声音的场景，直播观众端等默认需要接收频道内音频的场景，以及对远端音频首帧速度有较高要求的场景。
     *
     * > 注意：受浏览器自动播放策略影响，页面首次播放音频时仍可能需要用户进行点击或触摸操作。
     *
     * **自从**
