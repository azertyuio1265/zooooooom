...
export declare interface IAgoraRTC extends EventEmitter {
    ...
    /**
     * Performs network quality detection between the client and edge node before initiating a call.
     *
     * @since
     ...
     * > - To ensure the data accuracy in Agora Analytics, Agora recommends that you specify `uid` for each user and ensure it is unique.
     */
    startLastmileProbeTest(appid: string, channel: string, token: string | null, uid?: UID | null): Promise<LastmileProbeResult>;
    /**
     ...
     *
     * > After log upload is enabled, the SDK starts uploading logs 5 seconds after `join` is called, without waiting for the user to join the channel successfully.
     * @category Logger
     */
...
export declare interface IAgoraRTCClient extends EventEmitter {
    ...
    join(appid: string, channel: string, token: string | null, uid?: UID | null, options?: {
        ...
        autoReceiveAndPlayAudio?: boolean;
    ...
    /**
     * Sets the dual-stream mode and the video profile of the low-quality video stream on the sender side.
     *
     * @since
     * <br>&emsp;&emsp;&emsp;*4.24.1*</br>
     *
     * You can use this method to set the following dual-stream modes:
     * - `AUTO_SIMULCAST_MODE`: Low-quality stream adaptive mode. The sender does not actively send the low-quality stream. The sender automatically sends the low-quality stream upon receiving the request for the low-quality stream.
     * - `DISABLE_SIMULCAST_MODE`: Never send the low-quality stream.
     * - `ENABLE_SIMULCAST_MODE`: Always send the low-quality stream together with the high-quality stream.
     *
     * Compared with the original {@link enableDualStream}/{@link disableDualStream}:
     *  - Calling `setDualStreamMode` with `mode` set to `DISABLE_SIMULCAST_MODE` has the same effect as calling `disableDualStream`; calling `setDualStreamMode` with `mode` set to `ENABLE_SIMULCAST_MODE` has the same effect as calling `enableDualStream`.
     *  - Both `setDualStreamMode` and `enableDualStream`/`disableDualStream` can be called before or after joining a channel; if both methods are called, the settings in the latter method take effect.
     *
     * To ensure clear integration logic, we recommend that you choose one set of APIs for integration. Agora plans to gradually deprecate the original `enableDualStream` and `disableDualStream` in future releases.
     *
     * > The following limitations apply to `setDualStreamMode`. Contact support@agora.io to lift such limitations.
     * > - A low-quality stream request from **an audience** has no response from the host sender by default.
     * > - In scenarios interoperating with the Native SDK, a Native client requesting a low-quality stream from a Web host is not supported by default.
     *
     * @param mode The streaming mode.
     * @param streamParameter The profile of the low-quality video stream.
     */
    setDualStreamMode(mode: SIMULCAST_STREAM_MODE, streamParameter?: LowStreamParameter): Promise<void>;
    /**
 ...
 */
export declare interface IJoinOptions {
    /**
    ...
    autoSubscribe?: boolean;
    /**
     * Whether to automatically receive and try to play remote audio when joining the channel.
     * - `true`: Yes
     * - `false`: (Default) No
     *
     * After setting this parameter as `true`, the SDK automatically receives and tries to play remote audio when the user joins the channel, reducing the waiting time before remote audio is heard.
     *
     * This feature applies to scenarios such as voice chat rooms and online meetings where users need to hear audio as soon as possible after joining the channel, live streaming audience scenarios where receiving audio in the channel is required by default, and scenarios that require a faster first frame of remote audio.
     *
     * > Note: Due to browser autoplay policies, the first audio playback on a page may still require the user to click or touch the page.
     *
     * **Since**
     * <br>&emsp;&emsp;&emsp;*4.24.6*
     */
    autoReceiveAndPlayAudio?: boolean;
}
...
/**u
 * Pre-call network quality probe result, which reflects the overall quality of uplink and downlink networks.
 * @since
 * <br>&emsp;&emsp;&emsp;*4.24.1*</br>
 */
export declare interface LastmileProbeResult {
    /**
 ...
 */
declare type ScreenAudioTrackInitConfig = {
    /**
     * Whether to enable acoustic echo cancellation:
     * - `true`: Enable acoustic echo cancellation.
     * - `false`: Do not enable acoustic echo cancellation.
 ...
 * - `"265"`: Use H.265 for encoding. If there are clients in the channel that do not support H.265, the SDK automatically falls back to the existing codec.
 *
 * The support of different codecs in browsers is as follows:
 * - VP8 is not supported in Safari 12.1 or earlier.
 * - H.265 is supported in the following browsers:
 *   - Chrome 136 or later on macOS/Windows/Android.
 *   - Safari 18 or later on macOS.
 *   - Safari 18 or later on iOS.
 * - H.265 is not supported in the following browsers:
