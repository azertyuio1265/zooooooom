...
export declare enum AgoraRTCEvent {
    ...
    XLA_PEER_FIRST_VIDEO_FRAME = "xla_peer_first_video_frame",
    ...
    AB_TEST = "ab_test",
    ALARM_STATUS_NOTIFY = "alarm_status_notify"
}

export declare type AgoraRTCEventFirstRemoteFrame = AgoraRTCEvent.FIRST_AUDIO_DECODE | AgoraRTCEvent.FIRST_AUDIO_RECEIVED | AgoraRTCEvent.FIRST_VIDEO_DECODE | AgoraRTCEvent.FIRST_VIDEO_RECEIVED | AgoraRTCEvent.XLA_PEER_FIRST_VIDEO_FRAME;
...
export declare class AgoraRTCEventReport {
     ...
     * 内部上报接口
     */
    _send(type: string, data: any, isKey: boolean): void;
    /**
     * 内部上报接口
     */
    _sendApiInvoke(item: ApiInvokeMessage): boolean;
    sessionInit(sessionId: string, params: EventReportInitParams): void;
    reportRteDetail(sessionId: string): void;
    ...
    firstXLAPeerFirstVideoFrame(sessionId: string, params: EventXLAPeerFirstVideoFrameParams): void;
    ...
    abTest(sessionId: string, params: EventABTestParams): void;
     ...
     * 发送告警上报
     */
    reportAlarm(sessionId: string | null, params: EventReportAlarmParams): void;
    reportCreateClientPreconnect(params: EventReportCreateClientPreconnectParams): void;
    ...
    reportResourceTiming(url: string, sessionId: string, clientId?: string): void;
    extApiInvoke: (msgs: ExtensionApiInvokeMessage[]) => Promise<void>;
}

export declare type AgoraRTCEventUploadFirstRemoteFrame = AgoraRTCEventUploadType.FIRST_AUDIO_DECODE | AgoraRTCEventUploadType.FIRST_AUDIO_RECEIVED | AgoraRTCEventUploadType.FIRST_VIDEO_DECODE | AgoraRTCEventUploadType.FIRST_VIDEO_RECEIVED | AgoraRTCEventUploadType.XLA_PEER_FIRST_VIDEO_FRAME;
...
export declare enum AgoraRTCEventUploadType {
    ...
    RTE_DETAIL = "io.agora.pb.Wrtc.RteDetailStats",
    ...
    XLA_PEER_FIRST_VIDEO_FRAME = "io.agora.pb.Wrtc.XLAPeerFirstVideoFrame",
    ...
    AB_TEST = "io.agora.pb.Wrtc.ABTest",
    ALARM_STATUS_NOTIFY = "io.agora.pb.Wrtc.AlarmStatusNotify"
}

declare interface ApiFailureRecord {
    apiName: string;
    errorCode?: string;
    errorMessage?: string;
    count: number;
...
export declare type CreateClientPreconnectMode = "report" | "request";

export declare type CreateClientPreconnectTarget = "ap" | "sdpendpoint" | "sua";
...
export declare interface EventABTestParams {
    intSucc: number;
    isApplied: number;
    tag: string;
    feature: string;
 ...
 * 告警上报参数
 */
export declare interface EventReportAlarmParams {
    type: EventReportAlarmType;
    status: EventReportAlarmStatus;
    message?: string;
    extend?: any;
}

export declare enum EventReportAlarmStatus {
    RECOVER = 0,
    WARNING = 1,
    ERROR = 2
}

export declare enum EventReportAlarmType {
    CONFIG_DISTRIBUTE = "config-distribute-error"
}
...
export declare interface EventReportChooseServerParams {
    ...
    vid?: string;
    resourceTimingInfo?: string;
}

export declare interface EventReportCreateClientPreconnectMessage extends EventReportApiInvokeMessage {
    process: string;
}

export declare interface EventReportCreateClientPreconnectParams {
    targets: CreateClientPreconnectTarget[];
    mode: CreateClientPreconnectMode;
    origin: string;
}
...
export declare interface EventReportJoinGatewayMessage extends EventReportBase {
    ...
    installId: string;
    isABTestSuccess: number;
    gatewayIp: null | string;
    gatewayPort: null | string;
    isProxy: number;
}
...
export declare interface EventReportRteDetailMessage extends EventReportBase {
    eventType: AgoraRTCEvent.RTE_DETAIL;
    ip?: string;
    sourceHost?: string;
    sourcePort?: number;
    ua?: string;
 ...
 * 临时方案, 后续后端处理
 */
export declare interface EventReportRteDetailParams {
    lts: number;
    vid: string;
    uid: null | number;
    cid: number;
...
export declare interface EventXLAPeerFirstVideoFrameMessage extends EventReportBase {
    eventType: AgoraRTCEvent.XLA_PEER_FIRST_VIDEO_FRAME;
    peer: number;
    width: number;
    height: number;
    ssrc: number;
    ...
    videoSsrcNotify: number;
    /**
     * 客户订阅时延, subscribeStart - onAddVideoStream
     * 用于 分析 客户 订阅的逻辑，如果 subscribeDelay 比较大，则需要分析 客户 的订阅逻辑
     */
    subscribeDelayMs: number;
    subscribeStart: number;
    subscribeEnd: number;
    /**
     * 用于 分析 客户可以 play，但是没有 play 的情况
     * playDelayMs = playStart - subscribeEnd
     * 如果 playDelayMs 比较大，则需要分析 客户 的使用逻辑
     */
    playDelayMs: number;
    playStart: number;
    playEnd: number;
    ...
    /**
     * 本地第一帧渲染时间，web 渲染时间
     * 如果 客户自渲染，则 firstRender = Math.max(firstPreRender, subscribeEnd)
     * 如果 agora渲染，则 firstRender = Math.max(firstPreRender, playEnd)
     */
    firstRender: number;
    isPreSub: number;
    isPrePc: number;
    isPreInstantVideo: number;
    frameType?: string;
    rtpTimestamp?: number;
    ...
    /**
     * 4.24.1 新增，sdp end point 是否pc 链接成功
     */
    sdpEndPoint?: number;
}

export declare interface EventXLAPeerFirstVideoFrameParams {
    peer: number;
    width: number;
    height: number;
    ssrc: number;
...
export declare function _getApiFailureRecords(): ApiFailureRecord[];

/**
 * 对配置对象生成稳定的哈希值
 * 不受 key 顺序影响，只要 key-value 一致就产生相同哈希
 */
export declare function hashConfig(config: any): string;

/**
 * FNV-1a 32-bit 哈希，碰撞率低于原 DJB2 变体，性能相当
 */
 ...
 * 检查配置是否在黑名单中
 */
export declare function isConfigBlocked(config: any): boolean;
...
/**
 * 配置落地时调用
 * - 配置没变 + 再次落地 → 说明这个配置确实没问题，标记为已验证
 * - 配置变了 → 新配置覆盖老的，重新开始监控
 */
export declare function onConfigApplied(config: any): void;
 ...
 * 全局可用，不依赖 client 实例
 */
export declare function recordApiFailure(apiName: string, error?: AgoraRTCError_2): void;

export declare const report: AgoraRTCEventReport;

export declare function _reportAlarmFn(sessionId: string | null, params: EventReportAlarmParams): void;

/**
 * 对对象的 key 进行递归排序后序列化
 * 保证相同 key-value 不同顺序的对象产生相同的字符串
 */
