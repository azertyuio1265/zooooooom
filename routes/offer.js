...
router.post('/offer/create', authenticate, authorize(['teacher']), upload.single('thumbnail'), [
    ...
    try {
        ...
        if (insertError) {
            logger.error('❌ خطأ في إدخال الدرس:', insertError);
            return res.status(500).json({ 
                success: false, 
...
router.get('/offers', async (req, res) => {
    try {
        ...
        // ✅ تصفية الدروس لمنع إظهار البث المباشر أو المنتهي في قائمة الحجز
        const offers = (rawOffers || []).filter(offer => {
            return offer.status === 'upcoming' && !offer.stream_url && !offer.stream_started_at && !offer.completed_at;
        });
