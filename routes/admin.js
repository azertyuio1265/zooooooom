...
router.post('/approve-teacher/:id', [
    ...
    try {
        ...
        if (updateError) {
            logger.error('❌ خطأ في تحديث حالة الأستاذ:', updateError);
        ...
        try {
            ...
            logger.error('❌ خطأ في إرسال بريد القبول:', emailError);
        }
        ...
        try {
            await insert('notifications', {
                user_id: teacherId,
                user_type: 'teacher',
                title: '🎉 تهانينا! تم قبول حسابك كأستاذ معتمد',
                message: 'تمت مراجعة ملفك الشخصي ووثائقك وقبول حسابك بنجاح. يمكنك الآن إضافة الدروس والدورات والتفاعل في المنصة!',
                is_read: false,
                created_at: new Date().toISOString()
            });
        } catch (notifErr) {
            console.warn('⚠️ فشل إرسال إشعار القبول للأستاذ:', notifErr.message);
        }
        ...
        try {
            await sendPushNotification(
                teacher,
                '🎉 تهانينا! تم قبول حسابك كأستاذ معتمد',
                'تمت مراجعة ملفك الشخصي ووثائقك وقبول حسابك بنجاح. يمكنك الآن إضافة الدروس والدورات.'
            );
        } catch (pushErr) {
        ...
        logger.error('❌ خطأ في قبول الأستاذ:', error.message);
