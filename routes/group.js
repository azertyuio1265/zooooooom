...
router.post('/:id/messages', authenticate, upload.single('file'), async (req, res) => {
    const groupId = req.params.id;
    let { message, file_url, file_name, file_size, file_type } = req.body || {};
    const userId = req.user.userId;
    ...
    try {
        ...
        // محاولة أولى: إدراج مع أعمدة المرفقات
        if (file_url) {
            try {
                const { data, error } = await supabase
                    .from('group_messages')
        ...
        // محاولة بديلة أو إذا لم يكن هناك ملف: إدراج مع ترميز المرفق داخل نص الرسالة في حال عدم وجود الأعمدة
        if (!inserted) {
            const combinedMessage = file_url 
                ? (cleanMessage ? `${cleanMessage}\n<!--ATTACHMENT:${JSON.stringify({ file_url, file_name: file_name || 'ملف PDF', file_size, file_type: file_type || 'application/pdf' })}-->` : `<!--ATTACHMENT:${JSON.stringify({ file_url, file_name: file_name || 'ملف PDF', file_size, file_type: file_type || 'application/pdf' })}-->`)
                : cleanMessage;
